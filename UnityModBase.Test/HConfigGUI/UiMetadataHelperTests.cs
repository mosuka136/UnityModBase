...
namespace UnityModBase.Test.HConfigGUI
{
    public class UiMetadataHelperTests
    {
        ...
        // 组合声明：ConfigGui 指定槽位数量，多个带索引的滑条分别占据各自槽位。
        private class CompositeConfig
        {
            [ConfigGui(2)]
            [ConfigSlider(0, 0f, 100f, 1f)]
            [ConfigSlider(1, 10f, 50f, 0.5f)]
        ...
        [Fact]
        public void GetMetadata_WhenDeclarationCombinesConfigGuiWithIndexedSliders_ReturnsCompositeWithFilledSlots()
        {
            // Arrange
            ...
            // Assert
            var composite = Assert.IsType<UiConfigMetadata>(result);
        ...
        {
            // Arrange：静态声明取值抛出的异常应降级为“无元数据”，避免单个声明阻断整个配置界面。
