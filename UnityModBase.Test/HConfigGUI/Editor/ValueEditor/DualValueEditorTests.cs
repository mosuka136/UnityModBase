...
namespace UnityModBase.Test.HConfigGUI.Editor.ValueEditor
{
    ...
    /// 并复用全局注册表按槽位值类型选出的子编辑器绘制；元素写入合并为整体值写回父条目，
    /// 外部值变化会清空槽位暂存。注册表为进程级静态状态，每个测试开始前先释放清空。
    /// </summary>
    public class DualValueEditorTests
    {
        public DualValueEditorTests()
        {
        ...
        {
            // Act & Assert
            var exception = Assert.Throws<ArgumentNullException>(() => new DualValueEditor(null));
        ...
        public void Constructor_WithUnityGui_StoresProvider()
        {
            ...
            // Act
            var editor = new DualValueEditor(unityGuiMock.Object);
        ...
        public void DrawValue_WhenEntryOrContextIsNull_DoesNothing()
        {
            ...
            // Act
            editor.DrawValue(null, new GuiStateStore());
        ...
        public void DrawValue_WhenSubEditorMatches_DrawsBothSlotsOnceInsideExpandedHorizontalArea()
        {
            ...
            var entryMock = CreateDualEntryMock(
                ...
                new UiConfigMetadata(new IUiMetadata[] { sliderMetadata, null }));
            var drawnSlots = new List<IEntryBinding>();
            ValueEditorRegistry.RegisterEditor(CreateRecordingSubEditor(drawnSlots).Object);
        ...
        public void DrawValue_WhenSubEditorThrows_StillEndsCompositeHorizontalAreaAndSkipsRemainingSlot()
        {
            ...
            subEditorMock
                .Setup(x => x.DrawValue(It.IsAny<IEntryBinding>(), It.IsAny<GuiContext>()))
                .Throws(new InvalidOperationException("slot failed"));
            ValueEditorRegistry.RegisterEditor(subEditorMock.Object);
            ...
            unityGuiMock.Verify(x => x.EndHorizontal(), Times.Once);
            subEditorMock.Verify(x => x.DrawValue(It.IsAny<IEntryBinding>(), It.IsAny<GuiContext>()), Times.Once);
        }
        ...
        public void DrawValue_WhenSubEditorCommitsSlot_ParentReceivesMergedTuple()
        {
            ...
            var editor = new DualValueEditor(CreateCompositeAreaGuiMock().Object);
            var entryMock = CreateDualEntryMock(new ConfigEntryValue<int, string>(1, "a"));
            var subEditorMock = new Mock<IValueEditor>(MockBehavior.Strict);
            subEditorMock.Setup(x => x.CanEdit(It.IsAny<IEntryBinding>())).Returns(true);
            subEditorMock
                .Setup(x => x.DrawValue(It.IsAny<IEntryBinding>(), It.IsAny<GuiContext>()))
                .Callback<IEntryBinding, GuiContext>((slot, context) =>
                {
        ...
        public void DrawExtra_WhenCalledAfterDrawValue_ReusesSameSlotInstances()
        {
            ...
            subEditorMock
                .Setup(x => x.DrawValue(It.IsAny<IEntryBinding>(), It.IsAny<GuiContext>()))
                .Callback<IEntryBinding, GuiContext>((slot, _) => valueSlots.Add(slot));
            subEditorMock
                .Setup(x => x.DrawExtra(It.IsAny<IEntryBinding>(), It.IsAny<GuiContext>()))
                .Callback<IEntryBinding, GuiContext>((slot, _) => extraSlots.Add(slot));
            ValueEditorRegistry.RegisterEditor(subEditorMock.Object);
        ...
        public void GetEditor_AfterRegistered_ReturnsDualValueEditorForDualEntry()
        {
            ...
            var editor = new DualValueEditor(new Mock<IUnityGuiProvider>().Object);
            ValueEditorRegistry.RegisterEditor(editor);
            var entryMock = CreateDualEntryMock(new ConfigEntryValue<int, string>(1, "a"));
        ...
        private static DualValueEditor CreateEditor()
        {
            return new DualValueEditor(new Mock<IUnityGuiProvider>(MockBehavior.Strict).Object);
        }
        ...
        private static Mock<IValueEditor> CreateRecordingSubEditor(List<IEntryBinding> drawnSlots)
        {
            ...
            subEditorMock
                .Setup(x => x.DrawValue(It.IsAny<IEntryBinding>(), It.IsAny<GuiContext>()))
                .Callback<IEntryBinding, GuiContext>((slot, _) => drawnSlots.Add(slot));
