...
namespace UnityModBase.Test.HConfigGUI.Bindings
{
    ...
    public class DualValueSlotBindingTests
    {
        ...
        public void SlotView_WhenParentProvided_ExposesElementTypesKeysAndDisplayInfo()
        {
            ...
            var metadata = new UiConfigMetadata(new IUiMetadata[] { null, sliderMetadata });
        ...
        public void Metadata_WhenParentHasComboMetadata_MapsEachSlotByIndex()
        {
            ...
            var parentMock = CreateParentMock(
                ...
                new UiConfigMetadata(new IUiMetadata[] { null, sliderMetadata }));
        ...
        public void Metadata_WhenComboArrayShorterThanSlotCount_MissingSlotReturnsNull()
        {
            ...
            var parentMock = CreateParentMock(
                new ConfigEntryValue<int, string>(1, "a"),
                new UiConfigMetadata(new IUiMetadata[] { new UiSliderMetadata(0f, 10f, 1f) }));
        ...
        [Fact]
        public void ResetValue_ClearsOwnBufferAndWritesSlotTypeDefault()
        {
            // Arrange：缓冲中留有暂存输入时重置应一并丢弃，并把槽位元素恢复为类型默认值。
            var parentMock = CreateParentMock(new ConfigEntryValue<int, string>(9, "a"));
            var slot0 = new DualValueSlotBinding(parentMock.Object, 0);
            slot0.EditBuffer.SetValue("pending", false);
        ...
        [Fact]
        public void ResetValue_WhenSlotIsReferenceType_WritesNullAndKeepsOtherElement()
        {
            // Arrange：引用类型槽位的类型默认值为 null，重置只影响本槽位元素。
            var parentMock = CreateParentMock(new ConfigEntryValue<int, string>(9, "a"));
            var slot1 = new DualValueSlotBinding(parentMock.Object, 1);
