...
{
    /// <summary>
    /// 把双元素配置项（值为封闭 <see cref="ConfigEntryValue{T1, T2}"/>）的单个元素投影为独立配置项绑定，
    ...
    /// 保留为“无外层定界符”哨兵（见 <see cref="ConfigFileModel.SplitCompositeString"/>），不会出现在任何真实键名中；
    /// 因此槽位键不会与真实条目键冲突（例如枚举展开键 <see cref="GuiContext.ExpandedEnumKey"/> 按键匹配），两槽位之间也互不相同。
    /// 名称与说明透传父条目，使变更提示仍显示真实配置项；槽位子元数据从父条目的
    /// <see cref="UiConfigMetadata"/> 按下标解析，父元数据缺失或槽位未声明时为 null，子编辑器走通用分支。
    /// </remarks>
    internal class DualValueSlotBinding : IEntryBinding
    {
        ...
        /// <summary>
        /// 清除本槽位全部暂存输入，并把父条目双元素值中本槽位的元素恢复为槽位类型的默认值。
        /// </summary>
        public void ResetValue()
        {
            EditBuffer.Clear();
            WriteMergedValue(ValueType.IsValueType ? Activator.CreateInstance(ValueType) : null);
        }

        /// <summary>
        /// 创建双元素配置项的槽位绑定。
        /// </summary>
        /// <param name="parent">值为封闭 <see cref="ConfigEntryValue{T1, T2}"/> 类型的父配置项绑定。</param>
        ...
        /// <exception cref="ArgumentException"><paramref name="parent"/> 的值类型不是封闭的 ConfigEntryValue&lt;,&gt; 泛型。</exception>
        public DualValueSlotBinding(IEntryBinding parent, int slotIndex, Action onParentValueWritten = null)
        {
            ...
            if (parentValueType == null || !parentValueType.IsGenericType || parentValueType.ContainsGenericParameters || parentValueType.GetGenericTypeDefinition() != typeof(ConfigEntryValue<,>))
                throw new ArgumentException($"Parent entry value type must be a closed {typeof(ConfigEntryValue<,>).FullName} generic type, got {parentValueType?.FullName}.", nameof(parent));

            Parent = parent;
            SlotIndex = slotIndex;
            ValueType = parentValueType.GetGenericArguments()[slotIndex];
            _ownValueProperty = parentValueType.GetProperty(slotIndex == 0 ? nameof(ConfigEntryValue<int, int>.Value1) : nameof(ConfigEntryValue<int, int>.Value2));
            _otherValueProperty = parentValueType.GetProperty(slotIndex == 0 ? nameof(ConfigEntryValue<int, int>.Value2) : nameof(ConfigEntryValue<int, int>.Value1));
            Metadata = ResolveSlotMetadata(parent.Metadata, slotIndex);
            _onParentValueWritten = onParentValueWritten;
        }

        // 与父条目另一元素的当前已提交值合并为新的整体值写入；引用类型元素允许 null（仅用于槽位重置）。
        private void WriteMergedValue(object value)
        {
            var tuple = Parent.Value;
            var otherValue = tuple == null ? null : _otherValueProperty.GetValue(tuple, null);
            var newTuple = SlotIndex == 0 ? Activator.CreateInstance(Parent.ValueType, value, otherValue) : Activator.CreateInstance(Parent.ValueType, otherValue, value);
        ...
        {
            var metadatas = (parentMetadata as UiConfigMetadata)?.Metadatas;
