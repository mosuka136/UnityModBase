...
{
    /// <summary>
    /// 保存单个配置项来自一个或多个编辑控件的暂存值，并以单调递增序号确定最后一次输入。
    /// 缓冲区既保留无效文本用于回显，也阻止其写入配置；实例不提供线程安全保证。
    /// </summary>
    public class EntryEditBuffer
    {
        ...
        /// <param name="isValid">该输入是否可提交到底层配置。</param>
        ...
        /// </summary>
        /// <param name="entry">接收提交值的配置项绑定。</param>
        /// <param name="transform">写入前的可选值转换函数。</param>
        /// <returns>仅当目标配置值实际变化时返回 true。</returns>
        /// <exception cref="ArgumentNullException"><paramref name="entry"/> 为 null。</exception>
