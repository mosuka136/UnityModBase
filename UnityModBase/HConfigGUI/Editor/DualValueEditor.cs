...
{
    /// <summary>
    /// 双元素配置项的组合编辑器：把值为封闭 <see cref="ConfigEntryValue{T1, T2}"/> 的条目投影为两个
    /// <see cref="DualValueSlotBinding"/> 槽位绑定，并按槽位值类型复用全局注册表
    ...
    /// 避免过期回显或未到期的延迟提交覆盖最新值。配置服务层拒绝双元素值嵌套，槽位绑定也不会再匹配本编辑器。
    /// </remarks>
    internal class DualValueEditor : IValueEditor
    {
        // 父条目到槽位状态的弱表：槽位绑定与父条目同生命周期，配置界面重建绑定树后旧槽位随父条目一起回收。
        ...
        /// <exception cref="ArgumentNullException"><paramref name="unityGui"/> 为 <c>null</c>。</exception>
        public DualValueEditor(IUnityGuiProvider unityGui)
        {
            UnityGui = unityGui ?? throw new ArgumentNullException(nameof(unityGui));
        }

        /// <summary>
        /// 判断绑定的值类型是否为封闭的 <see cref="ConfigEntryValue{T1, T2}"/> 泛型（任意元素类型组合）；
        ...
        public bool CanEdit(IEntryBinding entry)
        {
            ...
            return valueType != null && valueType.IsGenericType && valueType.GetGenericTypeDefinition() == typeof(ConfigEntryValue<,>);
        }
        ...
        public void DrawValue(IEntryBinding entry, GuiContext context)
        {
            ...
            // 复合值区域整体占用配置名和重置按钮之间的剩余宽度；子编辑器再在区域内部
            // 按各自策略分配空间，避免紧凑控件直接参与外层布局后破坏重置按钮对齐。
        ...
        public void DrawExtra(IEntryBinding entry, GuiContext context)
        {
            ...
            var state = GetSlotState(entry);
            ValueEditorRegistry.GetEditor(state.Slot0).DrawExtra(state.Slot0, context);
            ValueEditorRegistry.GetEditor(state.Slot1).DrawExtra(state.Slot1, context);
        }

        private void DrawSlot(IEntryBinding slot, GuiContext context)
        {
            ValueEditorRegistry.GetEditor(slot).DrawValue(slot, context);
        }
        ...
        /// <summary>
        /// 释放编辑器；子编辑器归全局注册表所有，槽位绑定随父条目弱引用回收，此实现不持有需释放状态。
        /// </summary>
        public void Dispose()
        {
        }
