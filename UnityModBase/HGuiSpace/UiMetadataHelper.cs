...
namespace UnityModBase.HGuiSpace
{
    /// <summary>
    /// 从条目声明中提取通用 GUI 元数据。
    /// 把单个 <see cref="EntrySliderAttribute"/> 映射为滑条元数据；存在 <see cref="EntryGuiAttribute"/>
    /// 组合标记时，按各滑条声明指定的槽位索引聚合为 <see cref="UiCompositeMetadata"/>。
    /// </summary>
    public static class UiMetadataHelper
    {
        /// <summary>
        /// 根据配置管理器类型和配置键读取控件元数据。
        /// 识别 <see cref="EntrySliderAttribute"/> 与 <see cref="EntryGuiAttribute"/>；配置项没有受支持特性时返回 <c>null</c>。
        /// 无组合标记时仅转换属性上的第一个特性，而反射不保证多个特性间的顺序，声明多个受支持特性时结果不确定；
        /// 有组合标记时按各子声明指定的槽位索引聚合，没有子声明提供有效索引的槽位保持 <c>null</c>。
        /// 反射读取中的异常会记录日志并降级为“无元数据”，避免单个声明阻断整个可编辑界面；参数校验异常不受此降级保护。
        /// </summary>
        ...
        public static IUiMetadata GetMetadata(Type classType, IConfigEntry entry)
        {
            ...
            try
            {
                ...
                var multipleMetadata = attributes.FirstOrDefault(a => a is EntryGuiAttribute) as EntryGuiAttribute;
                if (multipleMetadata == null)
                    return GetMetadataFromAttribute(attributes.FirstOrDefault(a => a is IEntryGuiAttribute), out _);
                ...
                return new UiCompositeMetadata(metadataArray);
            }
            catch (Exception ex)
            {
        ...
        /// 当前仅识别 <see cref="EntrySliderAttribute"/>；其余类型（含 <c>null</c> 与 <see cref="EntryGuiAttribute"/> 标记本身）返回 <c>null</c>，
        ...
        private static IUiMetadata GetMetadataFromAttribute(Attribute attribute, out int index)
        {
            ...
            switch (attribute)
            {
                case EntrySliderAttribute slider:
                    index = slider.Index;
