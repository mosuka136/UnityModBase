...
{
    /// <summary>
    /// 把由 <see cref="DualValueDescriptor"/> 描述的双元素条目中的单个元素投影为独立条目绑定，
    ...
    internal class DualValueSlotBinding : IEntryBinding
    {
        ...
        /// <summary>
        /// 按 Value1/Value2 属性约定创建双元素条目的槽位绑定。
        /// </summary>
        /// <param name="parent">值类型为封闭双参数泛型并公开 Value1/Value2 属性的父条目绑定。</param>
        ...
        /// <exception cref="ArgumentException"><paramref name="parent"/> 的值类型与描述不匹配。</exception>
        public DualValueSlotBinding(IEntryBinding parent, int slotIndex, Action onParentValueWritten = null)
            : this(parent, slotIndex, CreateConventionalDescriptor(parent), onParentValueWritten)
        {
        }
        ...
        /// 使用明确的双元素值类型描述创建槽位绑定。
        /// </summary>
        /// <param name="parent">值类型与 <paramref name="descriptor"/> 匹配的父条目绑定。</param>
        /// <param name="slotIndex">元素下标，必须为 0 或 1。</param>
        /// <param name="descriptor">父条目的双元素值类型描述。</param>
        /// <param name="onParentValueWritten">本槽位向父条目写入整体值后的可选通知回调。</param>
            ...
            Action onParentValueWritten = null)
        {
            ...
            if (!_descriptor.CanDescribe(parentValueType))
                throw new ArgumentException($"Parent entry value type is not described by {_descriptor.GenericTypeDefinition.FullName}: {parentValueType?.FullName}.", nameof(parent));

            Parent = parent;
            SlotIndex = slotIndex;
            ValueType = _descriptor.GetSlotType(parentValueType, slotIndex);
            _ownValueProperty = _descriptor.GetSlotProperty(parentValueType, slotIndex);
            _otherValueProperty = _descriptor.GetSlotProperty(parentValueType, slotIndex == 0 ? 1 : 0);
            Metadata = ResolveSlotMetadata(parent.Metadata, slotIndex);
            _onParentValueWritten = onParentValueWritten;
        }

        private static DualValueDescriptor CreateConventionalDescriptor(IEntryBinding parent)
        {
            if (parent == null)
                throw new ArgumentNullException(nameof(parent));
            if (DualValueDescriptor.TryCreateConventional(parent.ValueType, out var descriptor))
                return descriptor;
            throw new ArgumentException($"Parent entry value type is not a conventional dual value type: {parent.ValueType?.FullName}.", nameof(parent));
        }
        ...
        private void WriteMergedValue(object value)
        {
            ...
            var newTuple = SlotIndex == 0
                ? _descriptor.CreateValue(Parent.ValueType, value, otherValue)
                : _descriptor.CreateValue(Parent.ValueType, otherValue, value);
