...
namespace UnityModBase.HClassAttribute
{
    ...
    /// （如带索引的 <see cref="EntrySliderAttribute"/>），各自声明在组合中的槽位，
    /// 由可编辑 GUI 展开为 <see cref="T:UnityModBase.HGuiSpace.UiCompositeMetadata"/>。
    /// 与其他 GUI 特性一样，只影响控件展示，不参与配置读取、持久化或值域验证。
    /// </summary>
    /// <remarks>
    /// 组合用法依赖在同一属性上重复标注子特性，因此子特性类型必须声明 <c>AllowMultiple = true</c>
    /// （当前仅 <see cref="EntrySliderAttribute"/> 支持）；新增可组合的子特性类型时同样需要放开该限制。
    ...
    public class EntryGuiAttribute : Attribute, IEntryGuiAttribute
    {
        ...
        public EntryGuiAttribute(int count)
        {
            if (count <= 0)
