...
namespace UnityModBase.HClassAttribute
{
    ...
    /// 该特性经通用 GUI 元数据解析器转换为控件元数据，不参与配置读取、持久化或值域验证。
    /// </summary>
    /// <remarks>
    /// 构造函数要求范围和步长均为有限数值，且最小值不得大于最大值；非正步长表示禁用吸附。
    /// 元数据在构造后不可变，可编辑 GUI 可直接使用已验证的范围和步长。
    /// 仅滑条交互会应用范围和步长，文本输入以及配置文件中的既有值仍可超出该范围。
    /// 除单独声明外，可经带索引的构造函数与 <see cref="EntryGuiAttribute"/> 组合，
    ...
    public sealed class EntrySliderAttribute : Attribute, IEntryGuiAttribute
    {
        ...
        /// <summary>
        /// 获取本声明在 <see cref="EntryGuiAttribute"/> 组合中的槽位下标；
        ...
        public EntrySliderAttribute(float min, float max, float step = -1f)
        {
        ...
        /// <summary>
        /// 创建用于 <see cref="EntryGuiAttribute"/> 组合声明中指定槽位的滑条特性。
        /// 范围与步长参数的约束与 <see cref="EntrySliderAttribute(float, float, float)"/> 相同。
        /// </summary>
        ...
        /// </remarks>
        public EntrySliderAttribute(int index, float min, float max, float step = -1f) : this(min, max, step)
        {
            Index = index;
        }
    }
